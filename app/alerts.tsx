...
export default function AlertsScreen() {
  ...
  const filtres = [
    ...
    { id: 'urgent', label: '🔴 Urgent', count: alertesData.filter(a => a.niveau === 'urgent').length },
    { id: 'warning', label: '🟠 Warning', count: alertesData.filter(a => a.niveau === 'warning').length },
    { id: 'info', label: '🔵 Info', count: alertesData.filter(a => a.niveau === 'info').length },
  ];
  ...
  const getNiveauStyle = (niveau) => {
    switch (niveau) {
      case 'urgent': return { bg: '#2C1010', border: '#E53935', badge: '#E53935' };
      case 'warning': return { bg: '#2C1E10', border: '#F57C00', badge: '#F57C00' };
      case 'info': return { bg: '#0D1E2C', border: '#1565C0', badge: '#1565C0' };
      default: return { bg: '#1E2D45', border: '#2A3F5F', badge: '#888' };
    }
  };
    ...
    >
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filtresContainer}>
        {filtres.map((f) => (
          <TouchableOpacity
            key={f.id}
            style={[styles.filtreBtn, filtre === f.id && styles.filtreBtnActive]}
            onPress={() => setFiltre(f.id)}
          >
            <Text style={[styles.filtreText, filtre === f.id && styles.filtreTextActive]}>
              {f.label} ({f.count})
            </Text>
          </TouchableOpacity>
        ))}
      ...
      <View style={styles.alertesList}>
        {alertesFiltrees.map((alerte) => {
          ...
          return (
            <View
              key={alerte.id}
              style={[styles.alerteCard, { backgroundColor: style.bg, borderColor: style.border }]}
            >
              ...
              <View style={styles.alerteCenter}>
                <View style={styles.alerteHeaderRow}>
                  <Text style={styles.alerteChambre}>Chambre {alerte.chambre}</Text>
                  <View style={[styles.niveauBadge, { backgroundColor: style.badge }]}>
                    <Text style={styles.niveauText}>{alerte.niveau.toUpperCase()}</Text>
                  </View>
                </View>
...
const styles = StyleSheet.create({
  ...
  alertBadge: { backgroundColor: '#E53935', borderRadius: 12, paddingHorizontal: 10, paddingVertical: 4 },
  ...
  resumeCard: {
    flex: 1, borderRadius: 14, padding: 15, alignItems: 'center',
  },
  ...
  filtreBtn: {
    ...
    backgroundColor: '#1E2D45', marginRight: 8, borderWidth: 1, borderColor: '#2A3F5F',
  },
  ...
  alerteCard: {
    ...
    borderWidth: 1.5, alignItems: 'flex-start', gap: 12,
  },
  alerteLeft: { paddingTop: 2 },
  alerteIcone: { fontSize: 28 },
  alerteCenter: { flex: 1 },
  alerteHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 },
  ...
  alerteMessage: { fontSize: 13, color: '#ccc', marginBottom: 6, lineHeight: 18 },
